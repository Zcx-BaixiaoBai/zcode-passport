...
typedef enum { LVL_WS = 0, LVL_SESS, LVL_DETAIL } level_t;
...
{
    wifi_sta_start(CONFIG_ZCODE_WIFI_SSID, CONFIG_ZCODE_WIFI_PASS);
    ui_set_busy("连接 WiFi…");
    if (!wifi_wait_connected(30)) {
        ui_set_busy("WiFi 连接失败\n检查 menuconfig 中\nZCODE_WIFI_SSID/PASS");
        // 继续留在遮罩态；长按 OK 重试
    } else {
        ui_set_busy(NULL);
        refresh_lists();
    ...
    for (;;) {
        ...
        switch (ev) {
        case EV_UP:
            if (s_level == LVL_DETAIL) ui_scroll_answer(28);
            else ui_list_move(-1);
            break;
        case EV_DOWN:
            if (s_level == LVL_DETAIL) ui_scroll_answer(-28);
        ...
        case EV_OKLONG:
            ...
            if (s_level == LVL_DETAIL) {
                ...
                refresh_lists();      // 顶级：长按 OK = 手动刷新/重试
            }
            break;
        }
    }
...
void app_main(void)
{
    ...
    gw_client_init(CONFIG_ZCODE_GW_URL, CONFIG_ZCODE_GW_TOKEN);
    xTaskCreate(badge_worker, "badge_worker", 12288, NULL, 5, NULL);
    ESP_LOGI(TAG, "worker 已启动，网关=%s", CONFIG_ZCODE_GW_URL);
}
