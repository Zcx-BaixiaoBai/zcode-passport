...
static esp_codec_dev_handle_t s_in_dev;    // 麦克风（输入）
static esp_codec_dev_handle_t s_out_dev;   // 扬声器（输出）
...
esp_err_t bsp_audio_init(void) {
    if (s_in_dev && s_out_dev) return ESP_OK;
    ...
    const audio_codec_if_t *codec = es8311_codec_new(&(es8311_codec_cfg_t){
        ...
        .use_mclk    = false,          // ★ 从 BCLK 取内部时钟（sunflower-buddy 验证过的配置；
                                       //   use_mclk=true 在本板 MCLK 未正确驱动时 ADC 无时钟→录音无数据）
        .hw_gain     = { .pa_voltage = 5.0f, .codec_dac_voltage = 3.3f },
    });
    if (!codec) { ESP_LOGE(TAG, "es8311_codec_new 失败"); return ESP_FAIL; }

    // ★ 拆成独立 IN/OUT 两个 codec_dev：分开 open/close，确保 mic 输入路径被显式激活
    s_out_dev = esp_codec_dev_new(&(esp_codec_dev_cfg_t){
        .dev_type = ESP_CODEC_DEV_TYPE_OUT, .codec_if = codec, .data_if = data,
    });
    s_in_dev = esp_codec_dev_new(&(esp_codec_dev_cfg_t){
        .dev_type = ESP_CODEC_DEV_TYPE_IN, .codec_if = codec, .data_if = data,
    });
    if (!s_in_dev || !s_out_dev) { ESP_LOGE(TAG, "esp_codec_dev_new 失败"); return ESP_FAIL; }
...
esp_err_t bsp_audio_set_format(uint32_t hz, uint8_t bits, uint8_t ch) {
    if (!s_in_dev) return ESP_ERR_INVALID_STATE;
    if (s_opened && s_hz == hz && s_bits == bits && s_ch == ch) return ESP_OK;   // 同格式复用

    if (s_opened) {
        esp_codec_dev_close(s_in_dev);
        esp_codec_dev_close(s_out_dev);
        s_opened = false;
    ...
    // ★ 先开输入（麦克风）并设增益，再开输出（扬声器），确保 mic 路径被激活
    if (esp_codec_dev_open(s_in_dev, &fs) != 0) {
        ESP_LOGE(TAG, "esp_codec_dev_open(in) 失败");
        return ESP_FAIL;
    }
    if (esp_codec_dev_set_in_gain(s_in_dev, 30.0f) != 0) {
        esp_codec_dev_close(s_in_dev);
        ESP_LOGE(TAG, "esp_codec_dev_set_in_gain 失败");
        return ESP_FAIL;
    }
    if (esp_codec_dev_open(s_out_dev, &fs) != 0) {
        esp_codec_dev_close(s_in_dev);
        ESP_LOGE(TAG, "esp_codec_dev_open(out) 失败");
        return ESP_FAIL;
    }
...
esp_err_t bsp_audio_write(const void *pcm, size_t bytes) {
    ...
    return esp_codec_dev_write(s_out_dev, (void *)pcm, bytes) == 0 ? ESP_OK : ESP_FAIL;
}

esp_err_t bsp_audio_read(void *pcm, size_t bytes) {
    if (!s_in_dev) return ESP_ERR_INVALID_STATE;
    return esp_codec_dev_read(s_in_dev, pcm, bytes) == 0 ? ESP_OK : ESP_FAIL;
}

esp_err_t bsp_audio_reopen(void) {
    if (!s_in_dev) return ESP_ERR_INVALID_STATE;
    uint32_t hz  = s_hz   ? s_hz   : 16000;
    uint8_t  bits = s_bits ? s_bits : 16;
    uint8_t  ch  = s_ch   ? s_ch   : 1;
    s_hz = 0;   // 使"同格式复用"短路失效，强制 set_format 走 close+open 刷新状态
    return bsp_audio_set_format(hz, bits, ch);
}

void bsp_audio_set_volume(uint8_t percent) {
    if (s_out_dev) esp_codec_dev_set_out_vol(s_out_dev, percent);
}
